...
def _check_nonce_length(nonce: bytes, name: str, algorithm: CipherAlgorithm) -> None:
    if not isinstance(algorithm, BlockCipherAlgorithm):
        raise UnsupportedAlgorithm(
            f"{name} requires a block cipher algorithm", _Reasons.UNSUPPORTED_CIPHER,
        )
...
def _check_iv_and_key_length(
    ...
    if not isinstance(algorithm, BlockCipherAlgorithm):
        raise UnsupportedAlgorithm(
            f"{self} requires a block cipher algorithm", _Reasons.UNSUPPORTED_CIPHER,
        )
...
class GCM(ModeWithInitializationVector, ModeWithAuthenticationTag):
    ...
    def validate_for_algorithm(self, algorithm: CipherAlgorithm) -> None:
        ...
        if not isinstance(algorithm, BlockCipherAlgorithm):
            raise UnsupportedAlgorithm(
                "GCM requires a block cipher algorithm", _Reasons.UNSUPPORTED_CIPHER,
            )
