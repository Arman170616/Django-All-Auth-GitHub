...
class _DHParameters(dh.DHParameters):
    ...
    def parameter_numbers(self) -> dh.DHParameterNumbers:
        ...
        return dh.DHParameterNumbers(
            p=self._backend._bn_to_int(p[0]), g=self._backend._bn_to_int(g[0]), q=q_val,
        )
    ...
    def parameter_bytes(
        self, encoding: serialization.Encoding, format: serialization.ParameterFormat,
    ) -> bytes:
...
class _DHPrivateKey(dh.DHPrivateKey):
    ...
    def _exchange_assert(self, ok: bool) -> None:
        if not ok:
            ...
            raise ValueError(
                "Error computing shared key.", errors_with_text,
            )
