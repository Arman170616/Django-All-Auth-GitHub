...
class Backend:
    ...
    def _register_default_ciphers(self) -> None:
        ...
        for cipher_cls, mode_cls in itertools.product(
            [_CAST5Internal, _IDEAInternal], [CBC, OFB, CFB, ECB],
        ):
            self.register_cipher_adapter(
                cipher_cls, mode_cls, GetCipherByName("{cipher.name}-{mode.name}"),
            )
    ...
    def _consume_errors_with_text(self,) -> typing.List[binding._OpenSSLErrorWithText]:
    ...
    def _crl_is_signature_valid(
        ...
        if not isinstance(
            public_key, (_DSAPublicKey, _RSAPublicKey, _EllipticCurvePublicKey,),
        ):
            raise TypeError(
    ...
    def _handle_key_loading_error(self) -> typing.NoReturn:
        ...
        if not errors:
            ...
            or errors[0]._lib_reason_match(
                self._lib.ERR_LIB_PKCS12, self._lib.PKCS12_R_PKCS12_CIPHERFINAL_ERROR,
            )
            or (
                self._lib.Cryptography_HAS_PROVIDERS
                and errors[0]._lib_reason_match(
                    self._lib.ERR_LIB_PROV, self._lib.PROV_R_BAD_DECRYPT,
                )
            )
        ):
        ...
        with self._zeroed_null_terminated_buf(password) as password_buf:
            ...
            if self._lib.Cryptography_HAS_PKCS12_SET_MAC and mac_alg != self._ffi.NULL:
                self._lib.PKCS12_set_mac(
                    p12, password_buf, -1, self._ffi.NULL, 0, mac_iter, mac_alg,
                )
    ...
    def pkcs7_serialize_certificates(
        self, certs: typing.List[x509.Certificate], encoding: serialization.Encoding,
    ):
        certs = list(certs)
        ...
        if encoding not in (serialization.Encoding.PEM, serialization.Encoding.DER,):
    ...
    def pkcs7_sign(
        ...
        p7 = self._lib.PKCS7_sign(
            self._ffi.NULL, self._ffi.NULL, certs, self._ffi.NULL, init_flags,
        )
    ...
    def __call__(self, backend: Backend, cipher: CipherAlgorithm, mode: Mode):
        ...
        if (
            ...
            evp_cipher = backend._lib.EVP_CIPHER_fetch(
                backend._ffi.NULL, cipher_name.encode("ascii"), backend._ffi.NULL,
            )
