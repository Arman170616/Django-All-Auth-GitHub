...
class _CMACContext:
    def __init__(
        self, backend: "Backend", algorithm: "ciphers.BlockCipherAlgorithm", ctx=None,
    ) -> None:
        if not backend.cmac_algorithm_supported(algorithm):
            raise UnsupportedAlgorithm(
                "This backend does not support CMAC.", _Reasons.UNSUPPORTED_CIPHER,
            )
        ...
        if ctx is None:
            ...
            res = self._backend._lib.CMAC_Init(
                ctx, key_ptr, len(self._key), evp_cipher, self._backend._ffi.NULL,
            )
